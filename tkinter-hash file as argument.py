...
def lecture(temp_path):
    f = open(temp_path, 'rb')
    text = f.read()
    f.close()
    return text

content=lecture(str(sys.argv[1]))
print(content)
...
#entree.pack()
entree.grid(column=0,row=2,columnspan=4,rowspan=1)
#zone texte réponse
text2 = StringVar()
text2.set("SHA1:")
entree2 = Entry(fenetre, textvariable=text2, width=130)
#entree2.pack()
entree2.grid(column=0,row=3,columnspan=4,rowspan=1)
Ok = True
oktext=entree.get()
#bouton radio 2
value = StringVar()
...
bouton4 = Radiobutton(fenetre, text="SHA512", variable=value, value=4,selectcolor='red',indicatoron=0,command=affiche)
...
bouton10.grid(column=1,row=5,columnspan=1)      # si grid pas de pack
#bouton10.pack()
